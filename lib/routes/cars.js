...
function setenvparam() {
    if (process.env.NODE_ENV && process.env.NODE_ENV === "test") {
        conusername = "tovlhixtdmbgcz";condatabase = "dfvspqpvd9vmc6";conhost = 'ec2-23-21-91-183.compute-1.amazonaws.com';conpassword = 'f48766c6c29f9b25108448b51c39d55084235c27d9352129da35c9cddbb78823';conssl = true;
    } else if (process.env.NODE_ENV === "offline") {
        conusername = "andela";condatabase = "andela";conhost = "localhost";conpassword = "";conssl = false;
    } else {
...
route.get("/api/v1/allcars", (req, res) => {
        ...
        function dmainsearch() {
            ...
            if (typeof req.query.min_price !== "undefined") {
                if (isNaN(req.query.min_price)) {
                    ...
                    return;
                } else theminprice = parseInt(req.query.min_price);
            } else theminprice = null;
            if (typeof req.query.max_price !== "undefined") {
                if (isNaN(req.query.max_price)) {
                    ...
                    return;
                } else themaxprice = parseInt(req.query.max_price);
            } else themaxprice = null;
            if (typeof req.query.status !== "undefined") thesstatus = req.query.status;else thesstatus = null;
            if (typeof req.query.body_type !== "undefined") thetype = req.query.body_type;else thetype = null;
            if (typeof req.query.state !== "undefined") thestate = req.query.state;else thestate = null;
            if (typeof req.query.manufacturer !== "undefined") themanu = req.query.manufacturer;else themanu = null;
            pool.query("SELECT * FROM postads" + " where CASE" + " WHEN $1::varchar IS NOT NULL THEN status = $1" + " ELSE 1=1" + " END and CASE" + " WHEN $2::int IS NOT NULL THEN price >=$2" + " ELSE 1=1" + " END and CASE" + " WHEN $3::int IS NOT NULL THEN price <= $3" + " ELSE 1=1" + " END and CASE" + " WHEN $4::varchar IS NOT NULL THEN state = $4" + " ELSE 1=1" + " END and CASE" + " WHEN $5::varchar IS NOT NULL THEN body_type = $5" + " ELSE 1=1" + " END and CASE" + " WHEN $6::varchar IS NOT NULL THEN manufacturer = $6" + " ELSE 1=1" + " END", [thesstatus, theminprice, themaxprice, thestate, thetype, themanu], (erro, result) => {
...
route.get("/api/v1/car/:carid", (req, res) => {
    ...
    pool.query("select * from postads where id = $1", [req.params.carid], (error, result) => {
        ...
        if (result.rows.length > 0) res.status(200).json({ "status": 200, "data": result.rows });else res.status(404).send({ "status": 404, "error": "The car was not found" });
    });
});
///////////////////////////
