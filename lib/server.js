...
function setenvparam() {
	if (process.env.NODE_ENV && process.env.NODE_ENV === "test") {
		conusername = "tovlhixtdmbgcz";condatabase = "dfvspqpvd9vmc6";conhost = 'ec2-23-21-91-183.compute-1.amazonaws.com';conpassword = 'f48766c6c29f9b25108448b51c39d55084235c27d9352129da35c9cddbb78823';conssl = true;thegmail = "automartcontacts@gmail.com";
	} else if (process.env.NODE_ENV === "offline") {
		conusername = "andela";condatabase = "andela";conhost = "localhost";conpassword = "";conssl = false;thegmail = "automartmail@gmail.com";
	} else {
		...
		pool.query("select * from allusers where email=$1", [req.params.email], (err, ress) => {
			if (ress.rows.length > 0) {
				function makeid(length) {
					var result = '';var characters = 'abcdefghijklmnopqrstuvwxyz';var charactersLength = characters.length;for (var i = 0; i < length; i++) {
...
app.get("/api/v1/me", ensureToken, function (req, res) {
	...
	jwt.verify(req.token, "ourlittlesecret", function (err, data) {
		if (err) {
			...
			res.status(200).json({ status: 200, description: data });
		}
	});
});
